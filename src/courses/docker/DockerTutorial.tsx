...
const DockerTutorial: React.FC<CourseComponentProps> = ({ onBack }) => {
  const [currentSlide, setCurrentSlide] = useState(0);
  const [expandedSteps, setExpandedSteps] = useState<{ [key: string]: boolean }>({});
  const [completedSteps, setCompletedSteps] = useState<{ [key: string]: boolean }>({});
  const [dockerfileContent, setDockerfileContent] = useState('');

  const currentSlideData = dockerCourseSlides[currentSlide];
  ...
  const handleNext = () => {
    if (currentSlide < dockerCourseSlides.length - 1) {
      ...
      <div className="max-w-4xl mx-auto">
        <div className="bg-white rounded-lg shadow-lg p-6 mb-6">
          <div className="flex items-center justify-between mb-4">
            <div className="flex items-center gap-3">
              ...
              <h1 className="text-3xl font-bold text-gray-800">Docker and Docker Compose</h1>
            </div>
          </div>
          <div className="w-full bg-gray-200 rounded-full h-2">
            <div
              className="bg-blue-600 h-2 rounded-full"
              style={{ width: `${((currentSlide + 1) / dockerCourseSlides.length) * 100}%` }}
            />
          </div>
          <p className="text-sm text-gray-600 mt-2">
            Step {currentSlide + 1} of {dockerCourseSlides.length}
          </p>
        </div>
                    ...
                    {isExpanded && (
                      <div className="p-4 space-y-3">
                        ...
                        <textarea
                          ...
                          value={dockerfileContent || step.dockerfile}
        ...
        <div className="flex justify-between">
          ...
          <div className="flex gap-4">
            ...
            <button
              onClick={handleNext}
              disabled={currentSlide === dockerCourseSlides.length - 1}
